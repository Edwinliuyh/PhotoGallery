...
public class ThumbnailDownloader extends HandlerThread {
    ...
    protected void onLooperPrepared() {
        mHandler = new Handler() {
            //Looper接收 Handler的message，然后在消息队列处理message
        	@Override
    ...
    /**
     * 使用FlickrFetchr从URL下载图片字节，然后再转换为位图
     */
    private void handleRequest(final ImageView imageView) {
        try {
            ...
            if (url == null) 
                return;
            // 确认URL已存在后，获得字节流数组，然后将字节流数组转为位图
            ...
            mResponseHandler.post(new Runnable() {
                public void run() {
                    if (requestMap.get(imageView) != url)
                        return;
                    //从requestMap中删除Token，然后将bitmap设置到Token上
